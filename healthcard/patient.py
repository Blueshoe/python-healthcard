...
class Patient(object):
    ...
    def __init__(self, xml):
        ...
        postal_address_node = tree.xpath('//vsd:PostfachAdresse', namespaces=namsespaces)
        if postal_address_node:
            self.postal_address = PostalAddress(postal_address_node[0], namsespaces)
        residential_address_node = tree.xpath('//vsd:StrassenAdresse', namespaces=namsespaces)
        if residential_address_node:
            self.residential_address = ResidenceAddress(residential_address_node[0], namsespaces)
